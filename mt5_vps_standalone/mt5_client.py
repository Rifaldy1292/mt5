...
# type: ignore
"""
MT5 Client & Session Manager Module (VPS Edition)
Handles MetaTrader 5 terminal initialization, credentials authentication,
Wine Linux path support, real-time metrics, active open positions, and lifecycle.
"""
...
def discover_terminals() -> List[str]:
    """Auto-detect candidate MetaTrader 5 terminal64.exe installations on Windows & Wine."""
    found: List[str] = []
    roots = []
    for key in ('PROGRAMFILES', 'PROGRAMFILES(X86)', 'LOCALAPPDATA', 'WINEPREFIX', 'HOME'):
        val = os.getenv(key, '').strip()
        if val and os.path.isdir(val):
            roots.append(Path(val))

    for root in roots:
        patterns = ['*/terminal64.exe', '*/*/terminal64.exe', '.wine/drive_c/Program Files/*/terminal64.exe']
    ...
    def connect(self, login: int, password: str, server: str, timeout: int = 60000) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            if path and os.path.isfile(path):
                init_success = bool(mt5.initialize(path=path, login=int(login), password=str(password), server=str(server), timeout=timeout, portable=True))
            else:
                init_success = bool(mt5.initialize(login=int(login), password=str(password), server=str(server), timeout=timeout, portable=True))
            ...
            if not init_success:
                ...
                if path and os.path.isfile(path):
                    mt5.initialize(path=path, portable=True)
                else:
